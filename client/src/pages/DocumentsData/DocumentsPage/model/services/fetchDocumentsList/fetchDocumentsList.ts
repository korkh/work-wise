...
import {
	getDocumentsPageOrder,
	getDocumentsPagePageNumber,
	getDocumentsPageSearch,
	getDocumentsPageSize,
	getDocumentsPageSort,
} from "../../selectors/getDocumentsPageSelectors";
...
interface FetchDocumentsListProps {
	replace?: boolean;
}
...
>("employeesPage/fetchDocumenstList", async (_, thunkApi) => {
	...
	try {
		...
		const response = await extra.api.get<EmployeeDocument[]>("/documents", {
			headers: {
