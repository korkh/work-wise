...
export function startRefreshTokenTimer(dispatch, token) {
    stopRefreshTokenTimer();
    const getTokenFromStorage = localStorage.getItem(TOKEN_LOCALSTORAGE_KEY);
    if (!getTokenFromStorage) {
        throw new Error("Token not found in local storage. Refresh token procedure aborted! Please login!");
    }
    ...
    if (timeout > 0) {
        refreshTokenTimeout = setTimeout(async () => {
            try {
                const response = await dispatch(refreshTokenMutation(formerJWT)).unwrap();
                if (response.token) {
                    const newToken = response.token;
                    localStorage.setItem(TOKEN_LOCALSTORAGE_KEY, JSON.stringify(newToken));
